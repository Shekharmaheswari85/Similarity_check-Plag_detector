...
class Register(Resource):
    def post(self):
        ...
        hashed_pw=bycrypt.hashpw(password.encode('utf8'),bycrypt.gensalt())
        users.insert({
            "Username":username,
            "Password":hashed_pw,
            "Tokens":6
        ...
        retJson = {
            "status":200,
            "msg":"You've successfully signed upto the API"
        }
        return jsonify(retJson)
...
def verifyPw(username,password):
    ...
    if bycrypt.hashpw(password.encode('utf8'),hashed_pw)==hashed_pw:
        return True
    else:
        return False
